...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None, past=None, history=None):
        if past is None:
            past_length = 0
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None, lm_labels=None, past=None, history=None):
        if history is None:
            hidden_states, presents = self.transformer(input_ids, position_ids, token_type_ids, past)
            lm_logits = self.lm_head(hidden_states[:, :-1, :])
        else:
            hidden_states, presents = self.transformer(input_ids, position_ids, token_type_ids, past, history)

        if (lm_labels is not None) and (history is None):
            # loss_fct = CrossEntropyLoss(ignore_index=-1)
            # loss = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), lm_labels.view(-1))       
            loss_fct1 = CrossEntropyLoss(ignore_index=-1, reduction = 'none')
            loss1 = loss_fct1(lm_logits.view(-1, lm_logits.size(-1)), lm_labels[:, 1:].contiguous().view(-1))
            loss1 = loss1.view(lm_labels.size(0), lm_labels.size(1) - 1)
            label_size = torch.sum(lm_labels[:, 1:] != -1, dim=1).type(loss1.type())
            loss = torch.sum(loss1)/torch.sum(label_size)
            ppl = torch.mean(torch.clamp(torch.exp(torch.sum(loss1, dim=1)/label_size), max=100000))
            correct = (lm_logits.max(dim = -1)[1] == lm_labels[:, 1:]).sum().item()
            total = label_size.sum().item()
            return loss, ppl, correct, total, torch.sum(loss1,dim=1)/label_size
        return presents
